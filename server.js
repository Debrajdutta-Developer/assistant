 ...
 if(url?.startsWith('/api/')){
  ...
  try{
   ...
   if(url==='/api/repo/inspect'&&req.method==='POST'){
    ...
    const metaResponse=await fetch(base,{headers,signal:AbortSignal.timeout(12000)});
    if(!metaResponse.ok)return send(res,metaResponse.status===404?404:502,{error:metaResponse.status===404?'Public repository not found.':'GitHub unavailable or rate limited.'});
    const repo=await metaResponse.json();
    const listResponse=await fetch(base+'/contents',{headers,signal:AbortSignal.timeout(12000)});
    const files=listResponse.ok?(await listResponse.json()).map(x=>({name:x.name,type:x.type})).slice(0,100):[];
   ...
   if(url==='/api/export'&&req.method==='POST'){
    const {type,title,content}=await body(req);
    if(!['docx','pptx'].includes(type)||typeof title!=='string'||!title.trim()||title.length>120||typeof content!=='string'||!content.trim()||content.length>16000)return send(res,400,{error:'Provide a title and up to 16000 characters of content.'});
