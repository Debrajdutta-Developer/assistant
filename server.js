...
const error=e=>{const m=String(e.message||e);if(m==='RUNTIME_SETUP_REQUIRED')return 'Jarvis needs an AI runtime. Install the local starter model or configure an online router in Advanced → Model.';if(m==='LOCAL_MODEL_UNAVAILABLE')return 'No local AI model is installed yet. Use Advanced → Model → Download offline starter model.';if(m==='ONLINE_NOT_CONFIGURED')return 'Online AI is not configured. Add a router key in the server environment, or use the offline model.';return /fetch failed|timeout|aborted/i.test(m)?'Model unavailable or timed out. Check the local model or router.':m.slice(0,200);};
http.createServer(async(req,res)=>{
 ...
 if(url?.startsWith('/api/')){
  ...
  try{
   ...
   if(url==='/api/status'&&req.method==='GET'){const runtime=await runtimeStatus();return send(res,200,{models:runtime.ollama.models,onlineConfigured:runtime.online.configured,deviceBridge:await fs.access(bridge).then(()=>true).catch(()=>false),runtime:runtime.mode});}
