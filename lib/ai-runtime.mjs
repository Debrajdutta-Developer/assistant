...
export function runtimeConfig() {
  return { local: { url: DEFAULT_OLLAMA, preferredModel: DEFAULT_MODEL }, router: { url: ROUTER_URL, type: isLocalRouter() ? '9Router' : 'OpenAI-compatible router' } };
}
...
async function routerFetch(path, options = {}, timeout = 45000) {
  ...
  const response = await fetch(`${ROUTER_URL}${path}`, { ...options, headers: { 'content-type': 'application/json', ...(options.headers || {}) }, signal: AbortSignal.timeout(timeout) });
...
export async function runtimeStatus() {
  ...
  return { mode, ollama: { available: ollamaDetected, models, preferredModel: DEFAULT_MODEL, preferredInstalled }, router: { ...router, url: ROUTER_URL, type: isLocalRouter() ? '9Router' : 'OpenAI-compatible router' }, fallback: preferredInstalled ? 'local-first → 9Router' : router.available ? '9Router' : 'needs-setup' };
}
