...
export const CAPABILITIES = {
  ...
  calendar_write: { label: 'Calendar Write', description: 'Create or modify calendar events after Google authorization.', oauth: 'google' }
};
...
export async function readPermissions() {
  try {
    ...
    return { ...Object.fromEntries(Object.keys(CAPABILITIES).map(k => [k, false])), ...value };
  } catch (e) {
    if (e.code === 'ENOENT') return Object.fromEntries(Object.keys(CAPABILITIES).map(k => [k, false]));
    throw e;
  }
}
